...
export function buildTray(parsed, card = null, options = {}) {
  const rows = [];
  ...
  const addSet = (id, label, oldValue, newValue, extras = {}) => {
    ...
    rows.push({
      ...
      inferred: extras.inferred ?? false,
      apply: true,
    });
  };
  ...
  // Some real messages (plain "update" pings) carry no date/time field at
  // all. Rather than leaving lastReferralAt stuck on a stale value forever
  // — which would defeat the whole ">24h" staleness feature — we fall back
  // to "now" (the message was clearly just sent) and flag the row as
  // inferred so it's never mistaken for something the resident wrote.
  //
  // Manual corrections (options.skipReferralTimestamp) skip this row
  // entirely — fixing a typo isn't a new referral, and bumping the
  // staleness clock on every correction would let the ">24h" filter be
  // gamed into looking fresher than the patient actually is.
  if (!options.skipReferralTimestamp) {
    const referralInferred = !parsed.referralDateTime;
    const referralAt = parsed.referralDateTime ?? new Date().toISOString();
    addSet('lastReferralAt', 'Referral date/time', card?.lastReferralAt, referralAt, {
      oldDisplay: card?.lastReferralAt ?? null,
      newDisplay: referralInferred
        ? `(not stated — using time pasted: ${new Date(referralAt).toLocaleString()})`
        : (parsed.referralDateTimeRaw ?? parsed.referralDateTime),
      inferred: referralInferred,
    });
  }
  ...
  // Default labels — overridden by parsed.text.<key>Label when the source
  // message used a different real header wording (e.g. "Update" or
  // "Update / Event" instead of "History"), so the tray shows what the
  // resident actually wrote rather than a mismatched hardcoded name.
  const textLabels = {
    assessment: 'Assessment', history: 'History', vitals: 'Vitals (verbatim)',
    physicalExam: 'Physical exam', neuroExam: 'Neuro exam',
    labsImaging: 'Labs & imaging', meds: 'Meds on board', plans: 'Plans',
  };
  for (const [key, defaultLabel] of Object.entries(textLabels)) {
    const newText = parsed.text?.[key];
    if (!newText) continue;
    const oldText = card?.text?.[key] ?? null;
    if (sameValue(oldText, newText)) continue;
    const label = parsed.text?.[`${key}Label`] ?? defaultLabel;
    rows.push({
